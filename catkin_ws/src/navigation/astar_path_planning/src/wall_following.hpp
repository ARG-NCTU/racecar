...
private:
  ...
  {
    ros::Time FUCK_TIME = ros::Time::now();
    res.clear();
    ...
    if (res.empty())
    {
      if (wait_reverse_cnt < wait_threshold)
      {
        ...
        ros::Publisher pub_stop_cmd = nh_.advertise<std_msgs::Bool>("/racecar/arrive", 1);
    ...
    ros::Time SHIT_TIME = ros::Time::now();
    ...
    ros::Time RAY_TIME = ros::Time::now();
    //std::cout << (RAY_TIME - SHIT_TIME).toSec() << ", " << (SHIT_TIME - FUCK_TIME).toSec() << std::endl;
    planned_path.header.frame_id = (use_odom == true ? MAP_FRAME : ROBOT_FRAME);
    pub_path.publish(planned_path);
    if (verbose)
...
public:
  WallFollower(ros::NodeHandle nh, ros::NodeHandle pnh) : nh_(nh), pnh_(pnh), count(0), count_reverse(0), stuck_counter(0), wait_reverse_cnt(0)
  {
    // Subscriber and publisher
    sub_grid = pnh_.subscribe("occupancy_grid", 1, &WallFollower::cbMap, this);
    //sub_grid = nh.subscribe("Omap", 1, &WallFollower::cbMap, this);
    pub_path = pnh_.advertise<nav_msgs::Path>("planned_path", 1);
    left_srv = nh.advertiseService("left_following", &WallFollower::cbLeft, this);
    ...
    if (verbose)
    {
      ...
      timer = pnh_.createTimer(ros::Duration(0.3), &WallFollower::pub_marker_arr_thread, this);
    }
  }
};
